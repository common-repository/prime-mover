...
{     
    ...
    /**
     * Set user adjustment columns
     * @param array $user_adj_columns
     */
    ...
    /**
     * Get user adjustment columns
     * @return array
     */
    public function getUserAdjColumns()
    {
        return $this->user_adj_columns;
    }
     ...
     * @return array
     */
    public function maybeAddUserAdjColumns($ret = [])
    {
        if (isset($ret['autouser_id_adjust'])) {
            return $ret;
        }
        
        $user_adj_columns = $this->getUserAdjColumns();
        if (is_array($user_adj_columns ) && !empty($user_adj_columns)) {
            $ret['autouser_id_adjust'] = $user_adj_columns;
        }
    ...
    public function computePrimaryKeys($ret = [], $clean_tables = [])
    {
        ...
        if (isset($ret['tbl_primary_keys']) || isset($ret['autouser_id_adjust'])) {
            return $ret;
        }        
        ...
        if (!empty($ret['original_blogid'])) {
            $source_blog_id = $ret['original_blogid'];
            $source_blog_id = (int)$source_blog_id;
        }
        
        $core_tables = $this->getSystemInitialization()->getCoreWpTables($source_blog_id);
        if ($source_blog_id) {
            /** DEVS only: https://docs.google.com/document/d/1UyDut9OqAWQZbJWZ_kZrrcKOPPMz1hlPxSPFZOL39ag/edit?usp=drive_link */
            $columns_user_adj_setting = apply_filters('prime_mover_custom_user_id_col', [], $source_blog_id, $ret, $clean_tables);            
        }
        ...
        foreach ($clean_tables as $table) {   
            ...
            $user_id_col = $this->queryCustomUserIdColumn($table, $primary_keys, $columns_user_adj_setting, $source_blog_id, $core_tables, $ret);
            if (is_array($user_id_col)) {
                foreach ($user_id_col as $v) {                    
        ...
        if (!empty($autouser_id_adjust)) {
            $ret['autouser_id_adjust'] = $autouser_id_adjust;
    ...
    /**
     * Query custom user ID columns
     * @param string $table
     * @param array $primary_keys
     * @param array $columns_user_adj_setting
     * @param number $source_blog_id
     * @param array $core_tables
     * @param array $ret
     * @return array
     */
    protected function queryCustomUserIdColumn($table = '', $primary_keys = [], $columns_user_adj_setting = [], $source_blog_id = 0, $core_tables = [], $ret = [])
    {
        ...
        if (!empty($primary_keys[$table]['primary_keys'][0])) {
            $primary_key = trim($primary_keys[$table]['primary_keys'][0], "`");
        }
        
        if (!is_array($core_tables) || !$primary_key || in_array($table, $core_tables) || !$source_blog_id) {
            return [];
        }       
        ...
        if (isset($columns_user_adj_setting[$table])) {
            $custom = $columns_user_adj_setting[$table];
        }
    ...
    /**
     * Process customer user ID columns
     * @param array $res
     * @param string $table
     * @param number $source_blog_id
     * @param array $ret
    ...
    {        
        ...
        foreach ($res as $columns) {  
            ...
            if (isset($columns['Type'])) {
                $colType = $columns['Type'];
            }
            
            $is_numeric = false;
            if ($colType && $this->getSystemFunctions()->isNumericKey($columns, $this->getIntTypes())) {
                $is_numeric = true;
            }
    ...
    /**
     * Get prefixless table name
     * @param string $table
     * @param number $source_blog_id
     * @param array $ret
     * @return string
     */
    protected function getPrefixLessTable($table = '', $source_blog_id = 0, $ret = [])
    {
        $prefix = '';
        $bp_tables = [];
