...
class PrimeMoverExporter implements PrimeMoverExport
{
    ...
    private function executeDumpUsingPHP($filter_export_data = false, $target_path = '', $clean_tables = [], $ret = [], $db_dump_start = 0, $blog_id = 0)
    {
        ...
        try {
            ...
            foreach ($clean_tables as $key => $table) {
                ...
                while ($dumped_rows = $this->triggerMySQLDumpPHP($mysql_server, $table, $filter_export_data, $target_path, $i, $batch_size, $clean_tables, $table_count, 
                    $primary_keys, $orderbykeys, $left_off, $table_processed, $pdoSettings, $ret)) {
    ...
    private function triggerMySQLDumpPHP($mysql_server = '', $table = '', $filter_export_data = false, $target_path = '', 
        $i = 0, $batch_size = 10, $clean_tables = [], $table_count = 0, $primary_keys = [], $orderbykeys = [], $left_off = [], $table_processed = 0, $pdoSettings = [], $ret = [])
    {        
        $dumpSettings = $this->getMySQLDumpPHPParameters([$table], $ret);
     ...
     * @return array[]
     */
    protected function getMySQLDumpPHPParameters($clean_tables = [], $ret = [])
    {
        /**
        ...
        $init_commands = [];
        if (isset($ret['prime_mover_ansi_quotes_enabled'])) {
            $init_commands = ["SET SESSION sql_mode=(SELECT REPLACE(@@sql_mode,'ANSI_QUOTES',''))"];
        }
